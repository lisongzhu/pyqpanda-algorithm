...
- **VQE（变分量子本征求解器）**  
  通过变分方法求解哈密顿量基态能量，广泛应用于量子化学模拟。

- **HHL（量子线性方程组求解算法）**  
  在稀疏且良好条件数的线性系统下，实现指数级加速求解 $ A\mathbf{x} = \mathbf{b} $。
...
使用 [Apache License 2.0](https://gitee.com/Tencent/APIJSON/blob/master/LICENSE)，对 公司、团队、个人 等 商用、非商用 都自由免费且非常友好，请放心使用和登记。
